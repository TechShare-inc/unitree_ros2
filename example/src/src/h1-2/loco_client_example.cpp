...
class UnitreeH1ControlNode : public rclcpp::Node {
 public:
  explicit UnitreeH1ControlNode(const std::map<std::string, std::string> &args)
      : Node("unitree_h1_control_node"), args_(args), client_(this) {
    // Process commands
  ...
  bool handleActionError(int32_t error_code) {
    ...
    UT_PRINT_ERR(error_code,
                 unitree::robot::h1::UT_ROBOT_LOCO_ERR_LOCOSTATE_NOT_AVAILABLE);
    UT_PRINT_ERR(error_code,
                 unitree::robot::h1::UT_ROBOT_LOCO_ERR_INVALID_FSM_ID);
    UT_PRINT_ERR(error_code,
                 unitree::robot::h1::UT_ROBOT_LOCO_ERR_ODOMSTATE_NOT_AVAILABLE);
    UT_PRINT_ERR(error_code,
                 unitree::robot::h1::UT_ROBOT_LOCO_ERR_INVALID_TASK_ID);
  ...
  void ProcessCommands() {  // NOLINT
    for (const auto &arg_pair : args_) {
      ...
      if (arg_pair.first == "set_phase") {
        std::vector<float> phase = stringToFloatVector(arg_pair.second);
        if (phase.empty()) {
          RCLCPP_ERROR(this->get_logger(), "Phase vector is empty");
          continue;
        }

        auto ret = client_.SetPhase(phase);
        if (!handleActionError(ret)) {
          RCLCPP_ERROR(this->get_logger(), "SetPhase failed (ret=%d)", ret);
          continue;
        }

        RCLCPP_INFO(this->get_logger(), "Set phase: %s",
                    arg_pair.second.c_str());
      }
...
int main(int argc, char const *argv[]) {
  ...
  if (args.size() == 0) {
    std::cout << "usage: h1_loco_client_example [--get_fsm_id | ...]"
              << std::endl;
    return 0;
  }
  auto node = std::make_shared<UnitreeH1ControlNode>(args);
  rclcpp::spin(node);
  rclcpp::shutdown();
